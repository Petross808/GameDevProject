...
public class EM_Fireball : MonoBehaviour, IEntityModifier
{
    ...
    // Register the attack in the EntityCombat to a Secondary slot and inject Damage multiplied by _amount and Cooldown multiplied by 0.5 to the power of _amount
    void Start()
    {
        if (TryGetComponent<EntityCombat>(out EntityCombat ec))
        {
            ...
            _attackData.Cooldown *= Mathf.Pow(0.5f, _amount - 1);
        }
    }
}
